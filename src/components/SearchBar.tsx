...
const SearchBar: React.FC<SearchBarProps> = ({ onSearch, categoryFilter }) => {
  ...
  useEffect(() => {
    if (userLocation && isLocationSet) {
      // Safety check: if userLocation is a JSON string, parse it to get the city
      let locationValue = userLocation;
      try {
        // Check if it's a JSON object stored as string
        if (userLocation.startsWith('{') || userLocation.startsWith('[')) {
          const parsed = JSON.parse(userLocation);
          locationValue = parsed.city || parsed.area || 'Unknown';
        }
      } catch (e) {
        // Not JSON, use as is
