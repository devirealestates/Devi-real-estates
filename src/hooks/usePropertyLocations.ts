          ...
          querySnapshot.docs.forEach(doc => {
            ...
            if (data.location && typeof data.location === 'string') {
              ...
              if (cleanLocation) {
                ...
                // Check for common separators to find city name (last part)
                const separatorMatch = cleanLocation.match(/[,\-|]\s*([^,\-|]+)$/);
                
                let cityName = '';
                
                if (separatorMatch) {
                  // Has separator - extract city (last part after separator)
                  cityName = separatorMatch[1].trim().replace(/\.+$/, '');
                } else {
                  // No separator - treat entire string as city
                  cityName = cleanLocation.trim().replace(/\.+$/, '');
                }
                
                // Normalize city name for grouping (case-insensitive deduplication)
                const normalizedCity = cityName.charAt(0).toUpperCase() + cityName.slice(1).toLowerCase();
                
                if (normalizedCity.length >= 2) {
                  cities.add(normalizedCity);
                  
                  // Add full location to this city's areas
                  if (!areas.has(normalizedCity)) {
                    areas.set(normalizedCity, new Set<string>());
                  }
                  areas.get(normalizedCity)?.add(cleanLocation);
                }
              }
            }
